...
async function analyzeAllBackground(ids: string[]) {
  ...
  for (const id of ids) {
    try {
      await analyzeNewsletterById(id)
      analyzed++
    ...
    emitAppEvent('analyze-all:progress', { current: analyzed + failed, total: ids.length, failed })
  }
  ...
  emitAppEvent('analyze-all:done', { analyzed, failed, total: ids.length })
}
